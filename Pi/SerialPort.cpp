...
{
   //char        devName[40];
   std::string devNameString = std::string("");
   unsigned    baudRate;
   ...
   if( inDevName[0] != '/' )
   {
      ...
      devNameString = std::string( "/dev/" );
   }

   //StrMaxCat( devName, inDevName, sizeof( devName ) );

   devNameString += std::string( inDevName );

   std::cout << "device: " << devNameString << std::endl;
   ...
   if( (m_fd = open( devNameString.c_str(), O_RDWR | O_EXCL )) < 0 )
   {
